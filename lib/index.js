  ...
  cssDeclarations = function(filePath, declarations, opt) {
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      return async.eachSeries(declarations, function(declaration, cb) {
        var basePath, imgPath, m, ref, sp, x, y;
        if (!coordinate && ((ref = declaration.property) === 'background-image' || ref === 'background')) {
          ...
          if (m != null ? m[2] : void 0) {
            if (opt.basePath) {
              basePath = path.resolve(opt.basePath);
            }
            imgPath = m[2];
            if (imgPath.indexOf('/') === 0) {
              if (basePath) {
                imgPath = path.join(basePath, imgPath);
              }
            } else {
            ...
            if (coordinate) {
              declaration.value = declaration.value.replace(URL_REGEXP, function(full, quote, url) {
                quote = quote || '';
                if (m[2].indexOf('/') === 0) {
                  return ("url(" + quote + "/") + path.relative(basePath, coordinate.sprite) + (quote + ")");
                } else {
                  return ("url(" + quote) + path.relative(path.dirname(filePath), coordinate.sprite) + (quote + ")");
                }
              });
              if (vwWidthValue) {
